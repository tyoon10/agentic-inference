...
*   **Engine:** `mistralai/mistral-small-4-119b-2603` (119B total / 6.5B active MoE, 256K context window)
...
*   **Why Mistral Small 4?** A unified hybrid model (instruct + reasoning + agentic coding + multimodal) where tool-calling is a first-class capability. When building autonomous loops, model hallucination on JSON signatures is the single biggest cause of pipeline failure; in these projects Small 4 held strict format adherence with reliable parallel tool invocation.
...
*   **The Frontier Tier (Score > 0.6):** Highly ambiguous, complex reasoning tasks are immediately routed to `mistralai/mistral-large-3-675b-instruct-2512` (675B total / 41B active Mixture-of-Experts). 
...
**Under these assumptions, routing cuts the monthly inference bill roughly in half.** Two honest caveats: the classifier call itself adds a small per-task overhead (bounded at 100 output tokens on the fast tier), and quality parity on the routed 80% depends on the threshold being tuned for the workload. The pattern is simple; the tuning is the work.
